...
#	endif
	...
	if (bAllowPing && m_udpDecoder.decode(inputData)
		&& m_udpDecoder.getMessageType() == Mumble::Protocol::UDPMessageType::Ping) {
		gsl::span< const Mumble::Protocol::byte > encodedPing = handlePing(m_udpDecoder, m_pingEncoder, true);
...
#	endif
				...
				// This may be a general ping requesting server details. These are unencrypted
				if (bAllowPing && m_udpDecoder.decode(gsl::span< Mumble::Protocol::byte >(encrypt, len))
					&& m_udpDecoder.getMessageType() == Mumble::Protocol::UDPMessageType::Ping) {
					ZoneScopedN(TracyConstants::ping_processing_zone);

					gsl::span< const Mumble::Protocol::byte > encodedPing =
						handlePing(m_udpDecoder, m_pingEncoder, true);
					...
					switch (m_udpDecoder.getMessageType()) {
						...
						case Mumble::Protocol::UDPMessageType::Ping: {
							...
							if (!pingData.requestAdditionalInformation && !pingData.containsAdditionalInformation) {
								...
								gsl::span< const Mumble::Protocol::byte > encodedPing =
									handlePing(m_udpDecoder, m_pingEncoder, false);
...
void Server::processMsg(ServerUser *u, Mumble::Protocol::AudioData audioData) {
	...
	m_audioReceivers.clear();

	if (audioData.targetOrContext == Mumble::Protocol::ReservedTargetIDs::ServerLoopback) {
		m_audioReceivers.forceAddReceiver(*u, Mumble::Protocol::AudioContext::Normal, audioData.containsPositionalData);
		...
		for (User *p : c->qlUsers) {
			...
			m_audioReceivers.addReceiver(*u, *pDst, Mumble::Protocol::AudioContext::Normal,
										 audioData.containsPositionalData);
		}
		...
		for (ServerUser *pDst : channel) {
			m_audioReceivers.addReceiver(*u, *pDst, Mumble::Protocol::AudioContext::Shout,
										 audioData.containsPositionalData);
		}
		// These users receive audio because someone is whispering to them
		for (ServerUser *pDst : direct) {
			m_audioReceivers.addReceiver(*u, *pDst, Mumble::Protocol::AudioContext::Whisper,
										 audioData.containsPositionalData);
		}
	...
	m_audioReceivers.preprocessBuffer();

	bool isFirstIteration = true;
	QByteArray tcpCache;
	for (bool includePositionalData : { true, false }) {
		std::vector< AudioReceiver > &receiverList = m_audioReceivers.getReceivers(includePositionalData);
		...
		if (!audioData.containsPositionalData) {
			m_audioEncoder.dropPositionalData();
		}
		...
		while (currentRange.begin != currentRange.end) {
			...
			if (isFirstIteration
				|| !Mumble::Protocol::protocolVersionsAreCompatible(m_audioEncoder.getProtocolVersion(),
																	currentRange.begin->getReceiver().uiVersion)) {
				m_audioEncoder.setProtocolVersion(currentRange.begin->getReceiver().uiVersion);

				// We have to re-encode the "fixed" part of the audio message
				m_audioEncoder.prepareAudioPacket(audioData);

				if (audioData.containsPositionalData) {
					m_audioEncoder.addPositionalData(audioData);
				}
		...
		if (m_tcpTunnelDecoder.decode(gsl::span< const Mumble::Protocol::byte >(
			...
			if (m_tcpTunnelDecoder.getMessageType() == Mumble::Protocol::UDPMessageType::Audio) {
				...
				if (ok) {
					...
					processMsg(u, std::move(audioData));
				}
			}
		}
